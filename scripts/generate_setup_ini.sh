...
# (doc/spec.md 3.2). --disable-check=missing-required-package,missing-
# depended-package is needed because this is a small overlay repository,
# not a full package set -- packages here can legitimately depend on
# packages that only exist in the official Cygwin repository, which
# mksetupini would otherwise flag as errors and refuse to write anything.
#
